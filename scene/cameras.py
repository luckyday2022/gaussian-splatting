...
class Camera(nn.Module):
    def __init__(self, resolution, colmap_id, R, T, FoVx, FoVy, primx, primy, image, guidance, depthmap, mask, 
                 image_name, uid,
        ...
        self.mask = None
        if mask is not None:
            self.mask = cv2.resize(mask, resolution)
            if self.mask.ndim != 2:
                self.mask = self.mask[..., 0]
            self.mask = torch.from_numpy(self.mask[None]).to(self.data_device)
